...
WHERE $Key = '$value'"
    ...
    if ($UseWMI) {
        $resource = $null
        [array]$resource = Get-CimInstance -ComputerName $CMServerFQDN -Namespace "ROOT\SMS\Site_$SiteCode" -ClassName SMS_R_System -Filter "ResourceID = $ResourceID"
        if ($resource) {    
            $Result = $resource | remove-ciminstance -confirm:$false 
...
WHERE $Key = '$value'"
    ...
    if ($UseWMI) {
        $resource = $null
        $resource = Get-CimInstance -ComputerName $CMServerFQDN -Namespace "ROOT\SMS\Site_$SiteCode" -ClassName SMS_R_System -Filter "ResourceID = $ResourceID"
        if($resource -eq $null)
        {
            return $false
        }
